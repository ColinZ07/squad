...
class QANet(nn.Module):
    ...
    def __init__(self, word_vectors, char_vectors, context_max_len, query_max_len,
        ...
        #Layer Declarations
        self.emb = layers_XL.Embedding(wemb_dim, cemb_dim, d_model)
        self.emb_enc = layers_XL.Encoder(4, num_head, d_model, d_head, d_inner = d_model * 4, k=7, dropout=0.1) #Hard coded
        self.cq_att = layers_XL.CQAttention(d_model=d_model)
        self.cq_resizer = layers_XL.Initialized_Conv1d(d_model * 4, d_model) #Foward layer to reduce dimension of cq_att output back to d_dim
        self.model_enc_blks = nn.ModuleList([layers_XL.Encoder(2, num_head, d_model, d_head, d_inner = d_model * 4, k=5, dropout=0.1) for _ in range(7)])
        self.out = layers_XL.QAOutput(d_model)
        self.drop = nn.Dropout(dropout)

        self._create_parameters()

    def _create_parameters(self):
            self.pos_emb = layers_XL.PositionalEmbedding(self.d_model)
